...
function renderOperator(result, token) {
  ...
  const evidenceSection = byId("operator-evidence-summary") || document.createElement("section");
  evidenceSection.id = "operator-evidence-summary"; evidenceSection.className = "card operator-card";
  const evidenceHeading = document.createElement("h2"); evidenceHeading.textContent = "Resolution Evidence";
  const evidenceGrid = document.createElement("div"); evidenceGrid.className = "kv-grid";
  [["Resolution Reason", result.resolution_reason || "Not resolved"], ["Provider Event", evidence.provider_event || "—"], ["Provider Status", evidence.provider_status || "—"], ["Matched Order ID", evidence.matched_order_id || "—"], ["Authoritative Payment ID", evidence.authoritative_payment_id || "—"], ["Client Reported Payment ID", evidence.client_payment_id || "—"], ["Client/Provider Discrepancy", evidence.client_provider_discrepancy ? "Yes" : "No"], ["Webhook Signature Verified", evidence.signature_verified === true ? "Yes" : evidence.signature_verified === false ? "No" : "Not applicable"], ["Contradiction / Mismatch", evidence.contradiction || "None recorded"]].forEach(([label, value]) => { const cell = document.createElement("div"); const name = document.createElement("span"); name.textContent = label; const content = document.createElement("strong"); content.textContent = value; cell.append(name, content); evidenceGrid.append(cell); });
  evidenceSection.replaceChildren(evidenceHeading, evidenceGrid); document.querySelector("#operator-content .left-stack").append(evidenceSection);
  const checks = [["Client payment ID received", eventTypes.has("CLIENT_REPORTED")], ["Razorpay webhook received", eventTypes.has("WEBHOOK_RECEIVED")], ["Webhook signature verified", evidence.signature_verified === true], ["Provider status recorded", Boolean(evidence.provider_status)], ["Payment matched", Boolean(evidence.matched_order_id)], [`Authoritative state: ${result.status}`, eventTypes.has("ATTEMPT_RESOLVED")]];
