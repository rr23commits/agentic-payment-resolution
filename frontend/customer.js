...
window.onRazorpayDismiss = (checkout) => { fetch("/checkout/client-cancel", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({intent_id: checkout.intent_id}), keepalive: true}).then(async (response) => { if (!response.ok) { $("cart-status").textContent = "Could not cancel checkout. Payment status remains protected; do not retry."; return; } const result = await loadIntent(checkout.intent_id); $("cart-status").textContent = result?.status === "ABANDONED" ? "Checkout cancelled. You may try again." : result?.status === "FAILED" ? "Payment failed; you may try again." : result?.status === "PENDING" || result?.status === "AMBIGUOUS" ? "Payment is still being confirmed. Do not retry." : "Could not refresh payment status. Do not retry."; }).catch(() => { $("cart-status").textContent = "Could not cancel checkout. Payment status remains protected; do not retry."; }); };
